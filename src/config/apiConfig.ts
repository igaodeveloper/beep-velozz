...
const API_BASE_URL =
  ...
  "https://app.logmanager.com.br/api";
const API_TOKEN =
  process.env.EXPO_PUBLIC_API_TOKEN || Constants.expoConfig?.extra?.apiToken;

// Validate token in production
if (!API_TOKEN && process.env.NODE_ENV === "production") {
  throw new Error(
    "❌ CRITICAL: API_TOKEN is not configured. Set EXPO_PUBLIC_API_TOKEN in your .env file.",
  );
}
