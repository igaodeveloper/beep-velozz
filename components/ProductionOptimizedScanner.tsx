...
import {
  ...
  useAppStateChange,
} from "@/src/utils/productionBootstrap";
 ...
 * - Cleanup on unmount
 */
export const ProductionOptimizedScanner =
  React.memo<ProductionOptimizedScannerProps>(
    ({ onPackageScan, onError, maxPackages = 100 }) => {
      // State
      ...
      // Cleanup management
      const { registerCleanup } = useProductionCleanup(
        "ProductionOptimizedScanner",
      );
      ...
      const processScan = useStableCallback(async (rawBarcode: string) => {
        try {
          ...
          console.log(
            `[ProductionOptimizedScanner] 📥 ENTRADA: "${rawBarcode}"`,
          );
          ...
          console.log(
            `[ProductionOptimizedScanner] 🔍 VALIDANDO: "${rawBarcode}"`,
          );
          const validation = validateBarcode(rawBarcode);
          console.log(
            `[ProductionOptimizedScanner] ✅ VALIDAÇÃO: ${validation.isValid ? "PASSOU" : "FALHOU"}`,
          );
          if (!validation.isValid) {
            const errorMsg = validation.errors?.[0] || "Invalid barcode";
            console.error(
              `[ProductionOptimizedScanner] ❌ ERRO VALIDAÇÃO: ${errorMsg}`,
            );
            setError(errorMsg);
          ...
          console.log(
            `[ProductionOptimizedScanner] 📋 BARCODE NORMALIZADO: "${barcode}"`,
          );

          // Identify package type using the correct identification function
          console.log(
            `[ProductionOptimizedScanner] 🎯 IDENTIFICANDO: "${barcode}"`,
          );
          const pkgInfo = identifyPackage(barcode);
          console.log(
            `[ProductionOptimizedScanner] 📊 RESULTADO: type="${pkgInfo.type}", matched=${pkgInfo.matched}`,
          );
          const type = pkgInfo.type;
          const price = packagePricingService.getPriceForType(type);
          console.log(
            `[ProductionOptimizedScanner] 💰 PREÇO: type="${type}" → price=${price}`,
          );
          ...
          console.log(
            `[ProductionOptimizedScanner] 📦 PACOTE CRIADO:`,
            newPackage,
          );
          ...
          // Play audio feedback
          console.log(
            `[ProductionOptimizedScanner] 🔊 TOCANDO ÁUDIO para type="${type}"`,
          );
          const audioType = getAudioTypeForPackage(type);
          console.log(
            `[ProductionOptimizedScanner] 🎵 AUDIO TYPE: ${audioType}`,
          );
          ...
          console.log(
            `[ProductionOptimizedScanner] ✅ PACOTE PROCESSADO: ${JSON.stringify(newPackage)}`,
          );
        } catch (err) {
       ...
       */
      const debouncedScan: (barcode: string) => void = useDebouncedCallback<
        (barcode: string) => void
      >((barcode: string) => processScan(barcode), 150);
      ...
      const handleBarcodeScanned = useCallback(
        ({ data }: { data: string }) => {
          if (data && data.trim()) {
            console.log(
              `[ProductionOptimizedScanner] 📷 BARCODE DETECTED: "${data}"`,
            );
       ...
       */
      const getAudioTypeForPackage = useCallback(
        (type: string): ScannerAudioType => {
          switch (type) {
            case "shopee":
      ...
      const renderItem = useCallback(
        ({ item, index }: { item: ScannedPackageItem; index: number }) => (
          ...
          {__DEV__ && (
            <View style={styles.debugBox}>
              <Text style={styles.debugText}>
                Env: {envConfig.environment} | Platform:{" "}
                {envConfig.performance.debounceMs}ms
              </Text>
            </View>
          )}
        </View>
