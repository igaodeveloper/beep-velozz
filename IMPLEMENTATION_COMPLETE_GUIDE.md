...
function MyComponent() {
  ...
  return (
    <View>
      ...
      <Text>Pontos: {gamification.formatPoints(gamification.profile?.totalPoints || 0)}</Text>
...
function FinancialComponent() {
  ...
  const handleSessionEnd = (session) => {
    ...
    console.log('Valor da sessão:', financial.formatCurrency(metrics.totalValue));
...
function LocalizedComponent() {
  ...
  return (
    <View style={{ flexDirection: localization.getTextAlign() }}>
      ...
      <Text>{localization.formatCurrency(125.50)}</Text>
...
export default function UnifiedDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'gamification' | 'financial' | 'settings'>('gamification');
