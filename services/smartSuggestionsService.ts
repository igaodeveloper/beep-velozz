...
export class SmartSuggestionsService {
  ...
  generateSmartSuggestions(
    ...
    const sequentialSuggestions = this.generateSequentialSuggestions(recentCodes, recentTypes);
    suggestions.push(...sequentialSuggestions);

    // 2. Sugestões baseadas em padrões detectados
    const patternSuggestions = this.generatePatternBasedSuggestions(recentCodes, recentTypes);
    suggestions.push(...patternSuggestions);

    // 3. Sugestões baseadas em histórico do operador
    if (operatorId) {
      const operatorSuggestions = this.generateOperatorBasedSuggestions(recentCodes, operatorId);
      suggestions.push(...operatorSuggestions);
    }

    // 4. Sugestões baseadas em similaridade
    const similaritySuggestions = this.generateSimilaritySuggestions(recentCodes, recentTypes);
    suggestions.push(...similaritySuggestions);

    // 5. Sugestões baseadas em contexto da sessão
    const contextSuggestions = this.generateContextualSuggestions(recentPackages);
  ...
  private generatePatternBasedSuggestions(
    ...
    for (const pattern of patterns) {
      if (pattern.nextPredicted) {
        const predictedType = this.predictPackageType(pattern.nextPredicted, types);
   ...
   */
  private generateOperatorBasedSuggestions(codes: string[], operatorId: string): SmartSuggestion[] {
    const suggestions: SmartSuggestion[] = [];
   ...
   */
  private generateContextualSuggestions(packages: ScannedPackage[]): SmartSuggestion[] {
    const suggestions: SmartSuggestion[] = [];
   ...
   */
  private rankAndFilterSuggestions(suggestions: SmartSuggestion[]): SmartSuggestion[] {
    // Remover duplicados
   ...
   */
  private predictPackageType(code: string, recentTypes: PackageType[]): PackageType {
   ...
   */
  private countPackageTypes(packages: ScannedPackage[]): Record<PackageType, number> {
   ...
   */
  private findDominantType(counts: Record<PackageType, number>): PackageType | null {
    let maxCount = 0;
   ...
   */
  private generateCodeForType(type: PackageType, referenceCode: string): string {
    const now = Date.now();
