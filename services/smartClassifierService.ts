...
class SmartClassifierService {
  ...
  async classifyCode(code: string): Promise<ClassificationResult> {
    ...
    try {
      ...
      console.log(
        `[SmartClassifier] Classification completed in ${processingTime}ms`,
      );
  ...
  private async ensemblePredict(
    code: string,
    features: CodeFeatures,
  ): Promise<Map<PackageType, number>> {
  ...
  private patternMatching(
    ...
    // Avulso patterns
    if (
      features.patterns.includes("letter_prefix") &&
      features.letterCount > features.digitCount
    ) {
   ...
   */
  private featureBasedPrediction(
    features: CodeFeatures,
  ): Map<PackageType, number> {
    const predictions = new Map<PackageType, number>();
  ...
  private historicalSimilarity(
    ...
    const similarCodes = this.trainingData
      .filter(
        (sample) => this.calculateSimilarity(features, sample.features) > 0.8,
      )
  ...
  private generateReasoning(
    ...
    reasons.push(
      `Confiança de ${Math.round(confidence * 100)}% baseada em análise ensemble`,
    );
  ...
  private async retrainModel(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[SmartClassifier] Model retrained to version ${this.modelVersion}`,
      );
    } catch (error) {
  ...
  private async evaluateModel(): Promise<void> {
    ...
    for (const sample of testSet) {
      ...
      if (prediction?.type === sample.type) {
        ...
        correctCounts.set(
          sample.type,
          (correctCounts.get(sample.type) || 0) + 1,
        );
      }
    }
    ...
    console.log(
      `[SmartClassifier] Model evaluation - Accuracy: ${Math.round(accuracy * 100)}%`,
    );
  }
  ...
  private calculateSimilarity(
    features1: CodeFeatures,
    features2: CodeFeatures,
  ): number {
    let similarity = 0;
    let factors = 0;
    ...
    similarity +=
      1 -
      Math.abs(features1.length - features2.length) /
    ...
    similarity +=
      1 -
      Math.abs(features1.digitCount - features2.digitCount) /
        Math.max(features1.digitCount, features2.digitCount);
    factors++;

    similarity +=
      1 -
      Math.abs(features1.letterCount - features2.letterCount) /
        Math.max(features1.letterCount, features2.letterCount);
    factors++;

    // Entropia
    similarity +=
      1 -
      Math.abs(features1.entropy - features2.entropy) /
        Math.max(features1.entropy, features2.entropy);
    factors++;
  ...
  private calculateFeatureStatistics(): Record<PackageType, CodeFeatures> {
    ...
    typeGroups.forEach((features, type) => {
      const avg: CodeFeatures = {
        length:
          features.reduce((sum, f) => sum + f.length, 0) / features.length,
        prefix: "", // Não calcula média de prefixo
        suffix: "", // Não calcula média de suffix
        digitCount:
          features.reduce((sum, f) => sum + f.digitCount, 0) / features.length,
        letterCount:
          features.reduce((sum, f) => sum + f.letterCount, 0) / features.length,
        hasSpecialChars:
          features.reduce((sum, f) => sum + (f.hasSpecialChars ? 1 : 0), 0) /
            features.length >
          0.5,
        patterns: [], // Não calcula média de patterns
        entropy:
          features.reduce((sum, f) => sum + f.entropy, 0) / features.length,
  ...
  private calculateFeatureSimilarity(
    ...
    similarity +=
      1 -
      Math.abs(features.length - avgFeatures.length) /
        Math.max(features.length, avgFeatures.length);
    factors++;

    similarity +=
      1 -
      Math.abs(features.digitCount - avgFeatures.digitCount) /
        Math.max(features.digitCount, avgFeatures.digitCount);
    factors++;

    similarity +=
      1 -
      Math.abs(features.letterCount - avgFeatures.letterCount) /
        Math.max(features.letterCount, avgFeatures.letterCount);
    factors++;

    similarity +=
      1 -
      Math.abs(features.entropy - avgFeatures.entropy) /
        Math.max(features.entropy, avgFeatures.entropy);
    factors++;
  ...
  private calculatePrecision(
    ...
    types.forEach((type, i) => {
      ...
      const predictedPositives = confusionMatrix.reduce(
        (sum, row) => sum + (row?.[i] || 0),
        0,
      );
      precision[type] =
  ...
  private async saveModelMetrics(): Promise<void> {
    try {
      await AsyncStorage.setItem(
        this.STORAGE_KEYS.MODEL_METRICS,
        JSON.stringify(this.modelMetrics),
      );
      await AsyncStorage.setItem(
        this.STORAGE_KEYS.MODEL_VERSION,
        this.modelVersion,
      );
