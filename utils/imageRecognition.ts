...
export interface DamageType {
  type: 'tear' | 'wet' | 'crush' | 'puncture' | 'stain' | 'missing_label' | 'other';
  confidence: number;
  bbox?: BoundingBox;
  description: string;
}
...
class PackageImageRecognition {
  ...
  private async simulateImageAnalysis(
    ...
    if (damageDetected) {
      ...
      const recommendations = this.generateRecommendations(damageTypes, severity);
      ...
      return {
        ...
        timestamp: Date.now(),
        imageUrl,
        recommendations: ['Pacote em bom estado', 'Prosseguir com conferência normal'],
      };
    }
  }
   ...
   */
  private calculateSeverity(damageTypes: DamageType[]): ImageAnalysisResult['severity'] {
    ...
    const hasCriticalDamage = damageTypes.some(d => criticalTypes.includes(d.type));
  ...
  private generateRecommendations(
    ...
    switch (severity) {
      case 'severe':
        recommendations.push('🚨 Pacote com dano severo - Separar para devolução');
        recommendations.push('📸 Tirar fotos adicionais para documentação');
        recommendations.push('⚠️ Notificar supervisor imediatamente');
        break;
        
      case 'moderate':
        recommendations.push('⚠️ Pacote com dano moderado - Verificar conteúdo');
    ...
    damageTypes.forEach(damage => {
      switch (damage.type) {
        case 'wet':
          recommendations.push('💧 Verificar se conteúdo foi afetado pela umidade');
          break;
        case 'tear':
   ...
   */
  validateImageQuality(imageUrl: string): Promise<{ valid: boolean; quality: number; issues: string[] }> {
  ...
  getAnalysisStats(): {
    ...
    const damageCount = this.analysisQueue.filter(r => r.damageDetected).length;
    const avgConfidence = total > 0 
      ? this.analysisQueue.reduce((sum, r) => sum + r.confidence, 0) / total 
      : 0;
    
    const severityDistribution = this.analysisQueue.reduce((acc, result) => {
