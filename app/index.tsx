...
const IndustrialScannerView = React.lazy(
  () => import("@/components/IndustrialScannerView"),
);
...
const DivergenceScreen = React.lazy(
  () => import("@/components/DivergenceScreen"),
);
...
const EmptyStateWelcome = React.lazy(
  () => import("@/components/EmptyStateWelcome"),
);
const MainLayout = React.lazy(() => import("@/components/MainLayout"));
const PackagePhotoCapture = React.lazy(
...
const ScreenTransition = React.lazy(
  () => import("@/components/ScreenTransition"),
);
const DeliveryBoyLoading = React.lazy(
  () => import("@/components/DeliveryBoyLoading"),
);
...
const SCREEN_ANIMATIONS = {
  ...
  scanning: {
    type: ANIMATION_TYPES.SLIDE,
    direction: DIRECTIONS.LEFT,
    duration: 150,
  },
  report: {
    type: ANIMATION_TYPES.GLIDE,
...
export default function App() {
  ...
  const changeScreenWithAnimation = useCallback(
    (newScreen: AppScreen, showLoading = false) => {
      if (showLoading) {
        setIsLoading(true);
        setTimeout(() => {
  ...
  const handlePackageScanned = useCallback(
    (pkg: ScannedPackage) => {
      ...
      const existingPackage = currentSession.packages.find(
        (p) => p.code === pkg.code,
      );
      if (existingPackage) {
