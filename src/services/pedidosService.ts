...
export class PedidosService {
  private static cache = new Map<string, { pedido: Pedido; timestamp: number }>();
  ...
  static async buscarPedidoPorCodigo(codigo: string): Promise<PedidoResponse> {
    ...
    try {
      ...
      const pedidoValidado = response.pedido ? PedidoValidator.sanitizePedido(response.pedido) : (null as any);
      ...
      return {
        pedido: null as any,
        encontrado: false,
        mensagem: 'Erro ao consultar pedido. Verifique sua conexão e tente novamente.',
      };
    }
  }

  // Listar pedidos
  static async listarPedidos(pagina = 1, limite = 50): Promise<ListaPedidosResponse> {
    try {
  ...
  static async buscarPedidosPorStatus(status: string, pagina = 1, limite = 50): Promise<ListaPedidosResponse> {
    try {
  ...
  static async buscarPedidosPorCliente(clienteId: string, pagina = 1, limite = 50): Promise<ListaPedidosResponse> {
    try {
      return await LogManagerApi.buscarPedidosPorCliente(clienteId, pagina, limite);
    } catch (error) {
  ...
  static async atualizarStatusPedido(pedidoId: string, statusId: string): Promise<Pedido> {
    try {
      const pedido = await LogManagerApi.atualizarStatusPedido(pedidoId, statusId);
