...
class SmartNotificationManager {
   ...
   */
  async processAIInsights(insights: PatternInsight[], session?: Session): Promise<void> {
    for (const insight of insights) {
      if (this.shouldNotify(insight)) {
        const notification = this.createNotificationFromInsight(insight, session);
   ...
   */
  async processSessionEvent(event: string, session: Session, data?: any): Promise<void> {
   ...
   */
  private createNotificationFromInsight(insight: PatternInsight, session?: Session): SmartNotification {
  ...
  private mapInsightType(type: PatternInsight['type']): SmartNotification['type'] {
  ...
  private mapSeverityToPriority(severity: PatternInsight['severity']): SmartNotification['priority'] {
    const mapping: Record<PatternInsight['severity'], SmartNotification['priority']> = {
      low: 'low',
      medium: 'normal',
      high: 'high',
      critical: 'urgent'
