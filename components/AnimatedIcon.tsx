...
interface AnimatedIconProps {
  ...
  size?: number;
  color?: string;
  animationType?: 'pulse' | 'bounce' | 'shake' | 'rotate' | 'scale' | 'glow' | 'none';
...
export function AnimatedIcon({
  ...
  const triggerHaptic = () => {
    ...
    else if (hapticType === 'medium') advancedHaptics.trigger({ type: 'medium' });
    else if (hapticType === 'heavy') advancedHaptics.trigger({ type: 'heavy' });
    else if (hapticType === 'selection') advancedHaptics.trigger({ type: 'selection' });
    else if (hapticType === 'success') advancedHaptics.trigger({ type: 'success' });
    else if (hapticType === 'error') advancedHaptics.trigger({ type: 'error' });
    else if (hapticType === 'warning') advancedHaptics.trigger({ type: 'warning' });
  };
    ...
    switch (animationType) {
      case 'pulse':
        scale.value = withRepeat(
          withSequence(
            withTiming(1.2, { duration: animationDuration / 2, easing: Easing.bezier(0.4, 0.0, 0.2, 1) }),
            withTiming(1, { duration: animationDuration / 2, easing: Easing.bezier(0.4, 0.0, 0.2, 1) })
          ),
          -1,
          true
        );
      ...
      case 'bounce':
        translateY.value = withRepeat(
          withSequence(
            withTiming(-15, { duration: animationDuration / 4, easing: Easing.bezier(0.68, -0.55, 0.265, 1.55) }),
            withTiming(0, { duration: animationDuration / 4, easing: Easing.bezier(0.68, -0.55, 0.265, 1.55) })
          ),
          -1,
          true
        );
      ...
      case 'rotate':
        rotate.value = withRepeat(
          withTiming(Math.PI * 2, { duration: animationDuration, easing: Easing.linear }),
          -1,
          false
        );
        break;
      ...
      case 'scale':
        scale.value = withSequence(
          withTiming(1.3, { duration: 200, easing: Easing.bezier(0.68, -0.55, 0.265, 1.55) }),
          withTiming(1, { duration: 200, easing: Easing.bezier(0.68, -0.55, 0.265, 1.55) })
        );
        break;
      ...
      case 'glow':
        glowOpacity.value = withRepeat(
          withSequence(
            withTiming(0.8, { duration: animationDuration / 2, easing: Easing.bezier(0.4, 0.0, 0.2, 1) }),
            withTiming(0, { duration: animationDuration / 2, easing: Easing.bezier(0.4, 0.0, 0.2, 1) })
          ),
          -1,
          true
        );
  ...
  const handlePress = () => {
    ...
    if (animationType === 'scale') {
      scale.value = withSequence(
        withTiming(0.8, { duration: 100, easing: Easing.bezier(0.4, 0.0, 0.2, 1) }),
        withTiming(1, { duration: 100, easing: Easing.bezier(0.4, 0.0, 0.2, 1) })
      );
    }
    onPress?.();
  };
...
export function AnimatedBarChartIcon(props: Omit<AnimatedIconProps, 'children'>) {
...
export function AnimatedHistoryIcon(props: Omit<AnimatedIconProps, 'children'>) {
...
export function AnimatedSettingsIcon(props: Omit<AnimatedIconProps, 'children'>) {
...
export function AnimatedPackageIcon(props: Omit<AnimatedIconProps, 'children'>) {
...
export function AnimatedLoadingIcon({ size = 24, color }: { size?: number; color?: string }) {
  const rotate = useSharedValue(0);
  const scale = useSharedValue(1);
...
export function AnimatedSuccessIcon({ size = 24, trigger }: { size?: number; trigger?: boolean }) {
  ...
  useEffect(() => {
    if (trigger) {
      ...
      rotate.value = withSequence(
        withTiming(0, { duration: 100 }),
        withTiming(Math.PI / 8, { duration: 200, easing: Easing.bezier(0.68, -0.55, 0.265, 1.55) }),
        withTiming(0, { duration: 200, easing: Easing.bezier(0.68, -0.55, 0.265, 1.55) })
      );
...
export function AnimatedErrorIcon({ size = 24, trigger }: { size?: number; trigger?: boolean }) {
  const scale = useSharedValue(0);
  const translateX = useSharedValue(0);
