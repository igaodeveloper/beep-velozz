...
describe('CacheService', () => {
  ...
  describe('Cleanup', () => {
    it('should cleanup expired items', async () => {
      ...
      mockAsyncStorage.getAllKeys.mockResolvedValue([`cache_${expiredKey}`, `cache_${validKey}`]);
      ...
      expect(mockAsyncStorage.removeItem).toHaveBeenCalledWith(`cache_${expiredKey}`);
      expect(mockAsyncStorage.removeItem).not.toHaveBeenCalledWith(`cache_${validKey}`);
    });
  });
});