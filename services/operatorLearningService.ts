...
export class OperatorLearningService {
  ...
  private updateOperatorProfile(event: ScanLearningEvent): void {
    ...
    if (!profile) {
      profile = this.createOperatorProfile(
        event.operatorId,
        "Unknown Operator",
      );
   ...
   */
  private updateScanPatterns(
    profile: OperatorLearning,
    event: ScanLearningEvent,
  ): void {
  ...
  private updateTimePatterns(
    ...
    if (profile.timePatterns[timeSlot].length > 50) {
      profile.timePatterns[timeSlot] =
        profile.timePatterns[timeSlot].slice(-50);
    }
  }
   ...
   */
  private updatePreferredSequences(
    profile: OperatorLearning,
    event: ScanLearningEvent,
  ): void {
    if (event.context.previousCode) {
      ...
      const existingIndex = profile.preferredSequences.findIndex((seq) =>
        seq.includes(sequence),
      );
  ...
  private updateCommonMistakes(
    ...
    if (
      !event.wasCorrect &&
      event.predictedType &&
      event.predictedType !== event.actualType
    ) {
  ...
  predictPackageType(
    ...
    const patternMatches = Array.from(profile.codePreferences.entries()).filter(
      ([prefCode]) => this.extractCodePattern(prefCode) === codePattern,
    );
    ...
    if (patternMatches.length > 0) {
      ...
      const confidence = Math.min(
        0.8,
        typeCounts.get(dominantType)! / patternMatches.length,
      );
    ...
    if (patternFrequency > 5) {
      ...
      const patternCodes = Array.from(profile.codePreferences.keys()).filter(
        (c) => this.extractCodePattern(c) === scanPattern,
      );
      ...
      if (patternCodes.length > 0) {
        ...
        const confidence = Math.min(
          0.7,
          typeCounts.get(dominantType)! / patternCodes.length,
        );
  ...
  getOperatorMetrics(operatorId: string): {
    ...
    const accuracy =
      profile.accuracyHistory.length > 0
        ? profile.accuracyHistory.reduce((sum, acc) => sum + acc, 0) /
          profile.accuracyHistory.length
        : 0;

    const averageSpeed =
      profile.speedHistory.length > 0
        ? profile.speedHistory.reduce((sum, speed) => sum + speed, 0) /
          profile.speedHistory.length
        : 0;
  ...
  getOperatorInsights(operatorId: string): string[] {
    ...
    if (profile.preferredSequences.length > 5) {
      insights.push(
        "🔄 Operador com bom reconhecimento de padrões sequenciais",
      );
    }
  ...
  exportLearningData(operatorId?: string): any {
    if (operatorId) {
      return {
        ...
        events: this.learningEvents.filter(
          (event) => event.operatorId === operatorId,
        ),
      };
    }
