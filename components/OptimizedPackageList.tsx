...
import React, { memo, useMemo, useCallback, useRef, useEffect, forwardRef } from 'react';
import {
  View,
  Text,
  FlatList,
...
import { useOptimizedListAnimation, shouldAnimate } from '@/utils/performanceOptimizer';
...
const PackageListItem = memo<{
  ...
  const containerStyle = useMemo(() => [
    ...
    {
      backgroundColor: isHighlighted ? colors.primary + '20' : colors.surface,
  ...
  ], [isHighlighted, colors, showTypeColors, item.type, compact, animatedStyle]);
...
const OptimizedPackageListComponent = forwardRef<OptimizedPackageListRef, OptimizedPackageListProps>(({
  packages,
  onPackagePress,
  onPackageLongPress,
  ...
  const { shouldUseStaggered, staggerDelay } = useOptimizedListAnimation(packages.length);

  // Memoiza dados para evitar re-renders
  const memoizedPackages = useMemo(() => packages, [packages]);

  // Memoiza funções de callback
  const memoizedOnPress = useCallback((pkg: ScannedPackage) => {
    onPackagePress?.(pkg);
  }, [onPackagePress]);

  const memoizedOnLongPress = useCallback((pkg: ScannedPackage) => {
  ...
  useEffect(() => {
    animatedValues.current = packages.map((_, index) => 
      new Animated.Value(shouldUseStaggered ? 0 : 1)
    );
  ...
  const renderItem = useCallback<ListRenderItem<ScannedPackage>>(({ item, index }) => {
    ...
    return (
      <PackageListItem
        ...
        compact={compact}
        colors={colors}
        animatedValue={animatedValues.current[index] || new Animated.Value(1)}
      />
    );
  ...
  const scrollToItem = useCallback((packageId: string) => {
