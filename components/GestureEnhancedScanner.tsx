...
const GestureEnhancedScanner: React.FC<GestureEnhancedScannerProps> = ({
  ...
  const onLongPressGestureEvent = useCallback(
    (event: any) => {
      const { state } = event.nativeEvent;
  ...
  const detectCircleGesture = useCallback(
  ...
  // Estilos dinâmicos
  const containerStyle = [styles.container, { backgroundColor: colors.bg }];
  ...
  const hintTextStyle = [styles.hintText, { color: colors.secondary }];
                  ...
                  {lastGesture && (
                    <Animated.View style={gestureFeedbackStyle}>
                      <Text style={feedbackTextStyle}>
                        {getGestureEmoji(lastGesture)}{" "}
                        {getGestureName(lastGesture)}
                      </Text>
                    </Animated.View>
                  )}
