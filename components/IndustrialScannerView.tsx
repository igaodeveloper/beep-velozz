...
export default function IndustrialScannerView({
  ...
  const isTablet = useMemo(() => {
    const aspectRatio =
      Math.max(windowWidth, windowHeight) / Math.min(windowWidth, windowHeight);
  ...
  const [scanQuality, setScanQuality] = useState<"excellent" | "good" | "poor">(
    "good",
  );
  ...
  const [lastScanStatus, setLastScanStatus] = useState<
    "success" | "error" | "duplicate" | "idle" | null
  >("idle");
  ...
  const generateSmartSuggestions = useCallback(() => {
    ...
    if (recentCodes.length >= 2) {
      ...
      if (firstCode) {
        ...
        const similarCodes = recentCodes.filter((code) =>
          code.startsWith(commonPrefix),
        );
    ...
    if (lastNumbers.length >= 2) {
      ...
      if (Math.abs(diff) === 1) {
        ...
        if (lastCodeIndex !== -1 && recentCodes[lastCodeIndex]) {
          suggestions.push({
            code: recentCodes[lastCodeIndex]!.replace(
              /\d+$/,
              String(nextNumber),
            ),
            confidence: 0.9,
  ...
  const assessScanQuality = useCallback(
    (scanResult: any) => {
      const qualityFactors = {
        speed:
          scanResult.processingTime < 200
            ? "excellent"
            : scanResult.processingTime < 500
              ? "good"
              : "poor",
        confidence:
      ...
      const qualityScore = Object.values(qualityFactors).filter(
        (q) => q === "excellent",
      ).length;
      setScanQuality(
        qualityScore >= 2 ? "excellent" : qualityScore >= 1 ? "good" : "poor",
      );
    },
    [scanHistory],
  );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      unloadSounds().catch((err) => {
        console.error(
          "[IndustrialScannerView] ❌ Erro ao descarregar sons:",
          err,
        );
      });
    };
  ...
  const handleBarcode = useCallback(
    async (event: any) => {
      ...
      if (
        barcodeLocked ||
        scanner.state === ScannerState.LIMIT_REACHED ||
        isProcessing
      ) {
        console.debug(
          `[ProfessionalScanner] Ignored: locked=${barcodeLocked}, limitReached=${scanner.state === ScannerState.LIMIT_REACHED}, processing=${isProcessing}`,
        );
        return;
      }
      ...
      if (scanned.startsWith("{") && scanned.endsWith("}")) {
        try {
          ...
          if (obj && typeof obj.id === "string") {
            console.debug(
              `[ProfessionalScanner] Extracted ID from JSON: ${obj.id}`,
            );
            scanned = obj.id;
          }
        } catch (error) {
          console.debug(
            `[ProfessionalScanner] JSON parse failed, using raw data`,
          );
        }
      }
      ...
      console.debug(
        `[ProfessionalScanner] Process result: success=${result.success}, reason=${result.reason}, type=${result.type}, time=${processingTime}ms`,
      );
  ...
  const handleManualSubmit = useCallback(async () => {
    ...
    if (result.success) {
      ...
      setManualError(
        errorMessages[result.reason as keyof typeof errorMessages] ||
  ...
  const reticleDimensions = useMemo(() => {
    ...
    if (isTablet) {
      ...
      const width = responsiveScale(
        Math.max(300, Math.min(baseWidth * 0.48, 400) / scale),
      );
      const height = responsiveScale(
        Math.max(220, Math.min(width * 0.65, 300) / scale),
      );
      return { width, height };
    } else if (isUltraWide) {
      // Ultra-wide: balanced reticle with high-res optimization
      const width = responsiveScale(
        Math.max(280, Math.min(baseWidth * 0.42, 340) / scale),
      );
      const height = responsiveScale(
        Math.max(200, Math.min(width * 0.75, 280) / scale),
      );
      return { width, height };
    } else {
      // Mobile: compact professional reticle with high-res optimization
      const width = responsiveScale(
        Math.max(240, Math.min(baseWidth * 0.58, 320) / scale),
      );
      const height = responsiveScale(
        Math.max(180, Math.min(width * 0.7, 260) / scale),
      );
      return { width, height };
    }
  ...
  const sessionAnalytics = useMemo(() => {
    ...
    return {
      ...
      efficiency:
        scanQuality === "excellent" ? 95 : scanQuality === "good" ? 80 : 60,
    };
  }, [scanner.counts, maxScans, sessionStartTime, scanQuality]);

  // Check if all limits reached
  const allLimitsReached = useMemo(() => {
    return Object.entries(maxScans).every(
      ([type, max]) => scanner.counts[type as keyof typeof maxScans] >= max,
    );
  ...
  const scanLineStyle = useAnimatedStyle(() => ({
    ...
    opacity: interpolate(scanLineAnim.value, [0, 0.5, 1], [0.3, 1, 0.3]),
  }));
  ...
  const panelTranslateY = useMemo(
    () => responsiveScale(isTablet ? 200 : 180),
    [isTablet, responsiveScale],
  );
  const iconSize = useMemo(
    () => responsiveScale(isTablet ? 18 : 16),
    [isTablet, responsiveScale],
  );
          ...
          >
            ...
            >
              {allLimitsReached
                ...
                : scanner.state === ScannerState.LIMIT_REACHED
                  ? "Limite"
                  : scanner.state === ScannerState.PAUSED
                    ? "Pausado"
                    : divergenceAccepted
                      ? "Divergência Aceita"
                      : "Escaneando"}
        ...
        {[
          ...
          <Animated.View
            ...
            style={{
              ...
              transform: [
                { rotate: `${corner.rotation}deg` },
                { scale: cornerPulseAnim },
              ],
            }}
          >
            <View
      ...
      >
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={{
              flex: 1,
              backgroundColor: manualInputExpanded
                ? statusColor
              ...
              paddingHorizontal: isTablet ? 20 : 16,
              borderWidth: 1,
              borderColor: manualInputExpanded
                ? statusColor
                : "rgba(255,255,255,0.2)",
              alignItems: "center",
            }}
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {sessionAnalytics.totalScans} /{" "}
                  {sessionAnalytics.totalTargets}
                </Text>
              </View>
              ...
              >
                ...
                <Text
                  style={{
                    color:
                      sessionAnalytics.efficiency >= 90
                        ? "#10b981"
                        : sessionAnalytics.efficiency >= 70
                          ? "#3b82f6"
                          : "#f59e0b",
                    fontSize: isTablet ? 16 : 14,
                    fontWeight: "700",
