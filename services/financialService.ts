...
export class FinancialService {
  ...
  calculateSessionFinancials(
    ...
    const packages = {
      ...
      mercadoLivre: session.packages.filter(
        (pkg) => pkg.type === "mercado_livre",
      ).length,
  ...
  private calculateBonuses(
    ...
    if (!session.hasDivergence) {
      bonuses.special = this.getBaseValue(session) * 0.1;
    }
  ...
  private calculatePenalties(session: Session): SessionFinancials["penalties"] {
    ...
    if (session.hasDivergence) {
      ...
      penalties.divergences =
        divergence * this.valueConfig.penalties.divergence;
    }
  ...
  private getPackageValue(type: PackageType): number {
    switch (type) {
      case "shopee":
        return this.valueConfig.shopee;
      case "mercado_livre":
        return this.valueConfig.mercadoLivre;
      case "avulso":
        return this.valueConfig.avulso;
      default:
        return 0;
    }
  }
  ...
  generateOperatorSummary(
    ...
    for (const session of filteredSessions) {
      ...
      totalBonuses += Object.values(financials.bonuses).reduce(
        (sum, bonus) => sum + bonus,
        0,
      );
      totalPenalties += Object.values(financials.penalties).reduce(
    ...
    const averagePerSession =
      filteredSessions.length > 0 ? totalEarnings / filteredSessions.length : 0;
    ...
    // Calcular eficiência
    const efficiency =
      totalDuration > 0 ? (totalPackages / totalDuration) * 60 : 0;
  ...
  private calculateGrowthRate(sessions: Session[]): number {
    ...
    const firstHalfValue = firstHalf.reduce(
      (sum, session) =>
        sum +
        session.packages.reduce(
          (pkgSum, pkg) => pkgSum + this.getPackageValue(pkg.type),
          0,
        ),
      0,
    );

    const secondHalfValue = secondHalf.reduce(
      (sum, session) =>
        sum +
        session.packages.reduce(
          (pkgSum, pkg) => pkgSum + this.getPackageValue(pkg.type),
  ...
  generateROIAnalysis(
    ...
    // Métricas de retorno
    const paybackPeriod =
      totalRevenue > 0 ? (totalInvestment / totalRevenue) * 30 : 0; // dias
    const roiPercentage =
      totalInvestment > 0
  ...
  generateFinancialReport(
    ...
    const totalRevenue = sessionFinancials.reduce(
      (sum, fin) => sum + fin.totalValue,
      0,
    );
    const totalCosts = sessionFinancials.reduce(
      (sum, fin) =>
        sum +
        Object.values(fin.penalties).reduce((sum, penalty) => sum + penalty, 0),
      0,
    );
    ...
    const efficiency =
      sessionFinancials.reduce((sum, fin) => sum + fin.efficiency, 0) /
      sessionFinancials.length;
    ...
    for (const session of filteredSessions) {
      for (const pkg of session.packages) {
        ...
        byPackageType[pkg.type as keyof typeof byPackageType].value +=
          this.getPackageValue(pkg.type);
      }
    }
  ...
  private calculateTrend(sessions: Session[], metric: string): number[] {
    ...
    return sessions.map((session, index) => {
      ...
      switch (metric) {
        case "revenue":
          return financials.totalValue;
        case "efficiency":
          return financials.efficiency;
        case "accuracy":
          return this.calculateAccuracy(session);
        case "speed":
          return financials.duration > 0
            ? (financials.packages.total / financials.duration) * 60
            : 0;
        default:
          return 0;
      }
  ...
  private generateInsights(
    ...
    const dominantType = Object.entries(byPackageType).sort(
      ([, a], [, b]) => (b as any).value - (a as any).value,
    )[0];
    insights.push(
      `📦 Tipo predominante: ${dominantType[0]} (${(dominantType[1] as any).value} pacotes)`,
    );

    // Insight de período
    const bestTimeSlot = Object.entries(byTimeSlot).sort(
      ([, a], [, b]) => (b as any).revenue - (a as any).revenue,
    )[0];
    insights.push(
      `⏰ Melhor período: ${bestTimeSlot[0]} (R$ ${(bestTimeSlot[1] as any).revenue.toFixed(2)})`,
    );
  ...
  private generateRecommendations(insights: string[], trends: any): string[] {
    ...
    if (insights.some((insight) => insight.includes("abaixo da média"))) {
      recommendations.push(
        "🎯 Considere treinamento adicional para melhorar a velocidade",
      );
    }

    if (insights.some((insight) => insight.includes("Excelente performance"))) {
      recommendations.push(
        "🏆 Mantenha o excelente trabalho e compartilhe suas melhores práticas",
      );
    }
  ...
  generateRevenueProjection(
    ...
    const avgSessionValue =
      historicalData.reduce((sum, fin) => sum + fin.totalValue, 0) /
      historicalData.length;
    const avgPackagesPerSession =
      historicalData.reduce((sum, fin) => sum + fin.packages.total, 0) /
      historicalData.length;
