...
export function useFastScanner(options: FastScannerOptions) {
  const { maxAllowedScans, debounceMs = 50, enableHaptics = true, enableAnalytics = true } = options;
  ...
  const maxScanCache = useMemo(() => ({ ...maxAllowedScans }), [maxAllowedScans]);
  
  // Verificação ultra-rápida de duplicatas
  const isDuplicate = useCallback((code: string): boolean => {
    return packageCache.has(code);
  }, [packageCache]);
  
  // Verificação ultra-rápida de limites
  const checkLimits = useCallback((type: PackageType): boolean => {
  ...
  const processScan = useCallback(async (rawCode: string): Promise<FastScannerResult> => {
    ...
    try {
      ...
      statsRef.current.averageProcessingTime = 
        statsRef.current.averageProcessingTime * (1 - alpha) + processingTime * alpha;
  ...
  const scanCode = useCallback((rawCode: string): Promise<FastScannerResult> => {
  ...
  const createPackage = useCallback((result: FastScannerResult): ScannedPackage | null => {
  ...
  const allLimitsReached = useMemo(() => {
    return Object.entries(maxAllowedScans).every(([type, max]) => 
      counts[type as keyof typeof counts] >= max
    );
