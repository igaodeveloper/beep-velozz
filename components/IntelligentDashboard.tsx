...
import {
  ...
  PatternInsight,
} from "@/utils/aiPatternRecognition";
import {
  smartNotificationManager,
  SmartNotification,
} from "@/utils/smartNotifications";
import {
  packageImageRecognition,
  ImageAnalysisResult,
...
export default function IntelligentDashboard({
  ...
  const [imageStats, setImageStats] = useState(
    packageImageRecognition.getAnalysisStats(),
  );
  const [apiStats, setApiStats] = useState(packageAPIIntegration.getAPIStats());
  const [selectedTab, setSelectedTab] = useState<
    "overview" | "insights" | "performance" | "quality"
  >("overview");
  ...
  useEffect(() => {
    if (currentSession) {
      aiPatternRecognition
        .analyzeSession(currentSession, sessions)
        .then(setInsights);
    }
  ...
  const metrics = useMemo(() => {
    ...
    const divergenceRate =
      completedSessions > 0 ? (divergentSessions / completedSessions) * 100 : 0;
    const completionRate =
      totalSessions > 0 ? (completedSessions / totalSessions) * 100 : 0;
    const avgPackagesPerSession =
      completedSessions > 0 ? totalPackages / completedSessions : 0;
  ...
  const chartData = useMemo(() => {
    ...
    const last7Days = sessions
      ...
      .slice(-7)
      .map((s) => ({
        day: new Date(s.startedAt).toLocaleDateString("pt-BR", {
          day: "2-digit",
        }),
    ...
    const marketplaceDistribution = sessions.reduce(
      (acc, session) => {
    ...
    const operatorPerformance = sessions.reduce(
      (acc, session) => {
    ...
    return {
      ...
      barChart: {
        ...
        datasets: [
          {
            data: Object.values(operatorPerformance).map(
              (op: any) => op.packages,
            ),
          },
        ],
      ...
      {[
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            borderRadius: 8,
            backgroundColor:
              selectedTab === tab.key ? colors.primary : "transparent",
            alignItems: "center",
          }}
        >
          <Text
  ...
  const renderOverviewMetrics = () => (
    ...
    >
      {[
        ...
        {
          label: "Taxa Divergência",
          value: `${metrics.divergenceRate.toFixed(1)}%`,
          color: "#FF6B6B",
        },
      ].map((metric, index) => (
        <View
        ...
        >
          <Text
            style={{ color: colors.secondary, fontSize: 12, marginBottom: 4 }}
          >
            {metric.label}
          </Text>
          <Text
          ...
          >
            ...
            >
              ...
              {insight.actionable && (
                <TouchableOpacity
                  onPress={() =>
                    Alert.alert("Ação Recomendada", insight.description)
                  }
                  style={{
  ...
  const renderCharts = () => (
    <View style={{ marginHorizontal: 16, marginTop: 16 }}>
      ...
      >
        ...
        >
          <Text style={{ color: colors.secondary }}>
            Gráfico de linhas (requer react-native-chart-kit)
          </Text>
        </View>
      </View>
      ...
      >
        ...
        >
          <Text style={{ color: colors.secondary }}>
            Gráfico de pizza (requer react-native-chart-kit)
          </Text>
        </View>
      </View>
    </View>
    ...
    <View style={{ marginHorizontal: 16, marginTop: 16 }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={{ color: colors.secondary }}>
            Analisados: {imageStats.totalAnalyzed}
          </Text>
          <Text style={{ color: "#FF6B6B" }}>
            Danos: {imageStats.damageDetected}
          </Text>
      ...
      {currentSession && (
        ...
        >
          ...
          <Text style={{ color: colors.text, fontSize: 16 }}>
            {currentSession.operatorName} - {currentSession.packages.length}/
            {currentSession.declaredCount} pacotes
          </Text>
        </View>
      )}
